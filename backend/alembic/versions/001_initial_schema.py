...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("bet_id", "user_id"),
    )
    ...
    # 12. comments (FK: bets, users, self-ref)
    op.create_table(
        "comments",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("bet_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("bets.id"), nullable=False),
    ...
    op.create_index(
        "idx_bets_deadline", "bets", ["deadline"],
        postgresql_where=sa.text("status = 'open'"),
    )
    op.create_index("idx_bet_positions_bet", "bet_positions", ["bet_id"])
    op.create_index(
    ...
    op.create_index(
        "idx_comments_bet", "comments", ["bet_id", "created_at"],
...
def downgrade() -> None:
    # Drop indexes first
    op.drop_index("idx_comments_bet", table_name="comments")
    ...
    op.drop_index("idx_bet_positions_bet", table_name="bet_positions")
    op.drop_index("idx_bets_deadline", table_name="bets")

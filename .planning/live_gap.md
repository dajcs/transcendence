# Live Gap Analysis
  ...
  1. **fixed**: uncontested resolved markets can close without paying winners.
  ...
  2. **fixed**: users can place bets after the deadline if the worker has not swept the market yet.
  ...
  3. **fixed**: user-supplied LLM API keys are stored plaintext.

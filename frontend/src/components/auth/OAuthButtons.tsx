...
const PROVIDER_INFO: Record<string, { label: string; bg: string; hover: string }> = {
  google: { label: "Continue with Google", bg: "bg-white border border-gray-300 text-gray-700 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200", hover: "hover:bg-gray-50 dark:hover:bg-slate-600" },
...
export default function OAuthButtons() {
  ...
  useEffect(() => {
    api
      .get<{ providers: string[] }>("/api/auth/oauth/providers")
    ...
    <div className="flex flex-col gap-2 w-full max-w-sm">
      <div className="relative my-2">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="bg-gray-50 px-2 text-gray-500 uppercase tracking-wide dark:bg-slate-900 dark:text-gray-400">or OAUTH</span>
        </div>
      </div>
